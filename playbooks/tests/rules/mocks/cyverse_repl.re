...
replEntityRename(*SrcObj, *DestObj) {
	writeLine('serverLog', 'replEntityRename(*SrcObj, *DestObj) called');
}
...
cyverse_repl_api_replica_close_post(*Instance, *Comm, *JsonInput) {
	writeLine(
		'serverLog', 'cyverse_repl_api_replica_close_post(*Instance, *Comm, *JsonInput) called' );
}
