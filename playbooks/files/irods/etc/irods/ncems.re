# NCEMS project policy
#
# Any data object put in the NCEMS project collection will be stored on the
# NCEMS resource server, and no other data objects may be stored there.
#
...
#
# This branch restricts the NCEMS resource to files in the NCEMS collection.
#
# Parameters:
...
#                                  to store a replica on the NCEMS resource that should be.
# XXX - ^^^
#
pep_resource_resolve_hierarchy_pre(*Instance, *Context, *OUT, *Op, *Host, *PARSER, *VOTE) {
	on (cyverse_blockRescReq(ncems_RESC, *Context.resc_hier, *Context.logical_path)) {
		*msg = 'CYVERSE ERROR: ' ++ *Context.logical_path ++ ' not allowed on ' ++ ncems_RESC ++ '.';
