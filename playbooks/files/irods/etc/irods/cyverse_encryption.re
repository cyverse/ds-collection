...
_ipcEncryptionCheckEncryptionRequiredForCollInternal(*Coll) {
	*coll = str(*Coll);
	...
	foreach (*record in SELECT COLL_NAME, DATA_NAME WHERE COLL_NAME == *coll || LIKE '*coll/%') {
		# all encrypted files will have ".enc" extension
...
_ipcEncryptionCheckEncryptionRequiredForColl(*SrcColl, *DstColl) {
	msiSplitPath(str(*DstColl), *parentColl, *collName);
	# check if parent coll has encryption::required avu
	if (_ipcIsEncryptionRequired(*parentColl)) {
...
_ipcEncryptionCopyAVUFromParentInternal(*Coll, *EncryptionMode) {
	foreach (*rec in SELECT COLL_NAME WHERE COLL_NAME == *Coll || LIKE '*Coll/%') {
		...
		*err = errormsg(
		...
		if (*EncryptionMode != '') {
			*err = errormsg(
				msiModAVUMetadata(cyverse_COLL, *rec.COLL_NAME, 'set', 'encryption::mode', *EncryptionMode, ''),
				*msg);
			if (*err < 0) { writeLine('serverLog', *msg); }
		}
	}
...
cyverse_encryption_api_data_obj_rename_pre(*Instance, *Comm, *DataObjRenameInp) {
	if (int(*DataObjRenameInp.src_opr_type) == 11) {  # data object
		...
		_ipcEncryptionCheckEncryptionRequiredForColl(
